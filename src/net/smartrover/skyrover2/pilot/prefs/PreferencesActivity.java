 ...
 *  > https://github.com/bitcraze/crazyflie-android-client/blob/master/src/se/bitcraze/crazyfliecontrol/prefs/PreferencesActivity.java
 */

package net.smartrover.skyrover2.pilot.prefs;

import net.smartrover.skyrover2.pilot.prefs.SelectConnectionDialogFragment.SelectSkyRoverDialogListener;
import net.smartrover.skyrover2.pilot.R;
import net.smartrover.skyrover2.pilot.UsbLinkAndroid;

import net.smartrover.skyrover2.roverlib.CrazyradioLink;
import net.smartrover.skyrover2.roverlib.CrazyradioLink.ConnectionData;
...
public class PreferencesActivity extends PreferenceActivity {
    ...
    private static final String LOG_TAG = "SkyRover2Control_Preferences";
...
//            PreferenceManager.setDefaultValues(getActivity(), R.xml.advanced_preferences, false);
        ...
        private void setInitialSummaries() {
            ...
            findPreference(KEY_PREF_RESET_BTN).setOnPreferenceClickListener(new OnPreferenceClickListener() {
                ...
                public boolean onPreferenceClick(Preference preference) {
                    ...
                    Toast.makeText(getActivity(), R.string.msg_resetting_to_default, Toast.LENGTH_SHORT).show();
                    return true;
                }
            });
        ...
        private void checkGyroSensors() {
            ...
            if (mSensorManager.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR) == null && mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER) == null) {
                ...
                mNoGyroSensor  = true;
                pref.setSummaryOff(getResources().getString(R.string.msg_no_gyro_accel_sensor));
        ...
        private void radioScan() {
            new AsyncTask<Void, Void, ConnectionData[]>() {
                ...
                protected void onPreExecute() {
                    //mProgress = ProgressDialog.show(getActivity(), "Radio Scan", "Searching for the SkyRover2...", true, false);
                	mProgress = ProgressDialog.show(getActivity(), getResources().getString(R.string.preferences_radio_scan)
                			, getResources().getString(R.string.preferences_radio_scan_summary), true, false);
...
//                      result[2] = new ConnectionData(125, 2);
                        ...
                        if (result != null && result.length > 0) {
                            if(result.length > 1){
                                // let user choose connection, if there is more than one SkyRover2
                            ...
                            Toast.makeText(getActivity(), getResources().getString(R.string.msg_no_conn_found), Toast.LENGTH_SHORT).show();
                        }
                    }
                }
            }.execute();
        ...
        private void showSelectConnectionDialog(final ConnectionData[] result) {
            ...
            //supply list of SkyRover2 connections as arguments
            Bundle args = new Bundle();
            String[] skyRoverArray = new String[result.length];
            for(int i = 0; i < result.length; i++){
            	skyRoverArray[i] = i + ": Channel " + result[i].getChannel() + ", Data rate " + mDatarateStrings[result[i].getDataRate()];
            }
            args.putStringArray("connection_array", skyRoverArray);
            selectConnectionDialogFragment.setArguments(args);
            selectConnectionDialogFragment.setListener(new SelectSkyRoverDialogListener(){
                @Override
                public void onClick(int which) {
                    setRadioChannelAndDatarate(result[which].getChannel(), result[which].getDataRate());
                }
            });
            selectConnectionDialogFragment.show(getFragmentManager(), "select_skyrover");
        }
